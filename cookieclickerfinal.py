...
def loadFeature():
    global gold, goldUpgradeNumber,autoClickerAmount,totalCookies, goldsForAutoClicker, goldsForGoldUpgrade, cheatCodeUsage, counterLabel, saveInterval, language, formatNumbers, lastStartTime
...
#Code to send the email with!
def sendEmail():
  server = smtplib.SMTP('smtp.gmail.com', 587)
  server.starttls()
  server.login('7colies@gmail.com', 'cciiomsciwpmzitv')
  msg = EmailMessage()
  msg['Subject'] = "Nouveau score de coopy!"
  msg['From'] = "7colies@gmail.com"
  msg['To'] = "info.emb5@inraci.be"
  #msg['To'] = "alusim8@gmail.com"

  with open('save.xlsx', 'rb') as f:
      file_data = f.read()
  msg.add_attachment(file_data, maintype="application", subtype="xlsx", filename="save.xlsx")

  try:
    server.send_message(msg)
  except exception as e:
    return messagebox.showinfo("??", f"Error {e}")
  messagebox.showinfo("", f"{localization[language]['sent']}")
"------------------------------------VARIABLES------------------------------------"
